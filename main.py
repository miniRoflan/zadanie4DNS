...
class DNSServer:
    ...
    def make_answer(self, bytes):
        ...
        for question in msg.questions:
            if question not in self.cache or self.cache[question].exp_time < int(time.time()):
                return self.from_forwarder(bytes)
...
if __name__ == '__main__':
    server = DNSServer(53, ('8.26.56.26', 53), 'cache')
    try:
        server.start()
